...
impl<'a> Trackpad<'a> {
    ...
    fn set_cs(&self, high: bool) {
        ...
        match port {
            0 => {
                if high {
                    self.gpio0.data.modify(|r, w| unsafe { w.bits(r.bits() | (1 << pin)) });
                } else {
                    self.gpio0.data.modify(|r, w| unsafe { w.bits(r.bits() & !(1 << pin)) });
                }
            },
            1 => {
                if high {
                    self.gpio1.data.modify(|r, w| unsafe { w.bits(r.bits() | (1 << pin)) });
                } else {
                    self.gpio1.data.modify(|r, w| unsafe { w.bits(r.bits() & !(1 << pin)) });
                }
            },
    ...
    /// Apply smoothing filter to trackpad data
    pub fn apply_smoothing(&mut self, current: TrackpadData, previous: TrackpadData, alpha: f32) -> TrackpadData {
        if !current.touching || !previous.touching {
            return current;
        }
        
        let x_filtered = (alpha * (current.x as f32) + (1.0 - alpha) * (previous.x as f32)) as i16;
        let y_filtered = (alpha * (current.y as f32) + (1.0 - alpha) * (previous.y as f32)) as i16;
        let pressure_filtered = (alpha * (current.pressure as f32) + (1.0 - alpha) * (previous.pressure as f32)) as u8;
        
        TrackpadData {
            x: x_filtered,
            y: y_filtered,
            pressure: pressure_filtered,
            touching: current.touching,
        }
    }
    
    /// Get trackpad data with noise filtering
    pub fn get_filtered_trackpad_data(&mut self) -> Result<TrackpadData, ()> {
        let raw_data = self.to_trackpad_data()?;
        
        // Simple noise gate - ignore very small movements
        let noise_threshold = 50;
        if raw_data.touching && (raw_data.x.abs() < noise_threshold && raw_data.y.abs() < noise_threshold) {
            return Ok(TrackpadData {
                x: 0,
                y: 0,
                pressure: raw_data.pressure,
                touching: raw_data.touching,
            });
        }
    ...
    /// Detect if trackpad is being tapped (quick touch and release)
    pub fn detect_tap(&mut self, _touch_duration_ms: u32) -> bool {
        // This would need to track touch timing
        // For now, return true if pressure is above threshold but not too high
        match self.read_position() {
            Ok(pos) => pos.touching && pos.z > 30 && pos.z < 200,
            Err(_) => false,
        }
    }
    
    /// Get trackpad resolution info
    pub fn get_resolution(&self) -> (u16, u16) {
        // TM040040 typical resolution
        (1024, 1024)
    }
    
    /// Check if trackpad is healthy
    pub fn is_healthy(&mut self) -> bool {
        // Try to read device ID
        match self.read_register(REG_DEVICE_ID) {
            Ok(id) => id == TRACKPAD_DEVICE_ID,
            Err(_) => false,
        }
    }
    
    /// Force recalibration of trackpad

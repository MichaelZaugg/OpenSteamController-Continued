...
pub fn set_pin_mode(pin: Pin, val: PinMode) -> Result<(), PinError>
{
    ...
    let dp = match lpc11xx::Peripherals::take() {
...
pub fn gpio_digital_read(pin: Pin) -> Result<PinValue, PinError>
{
    ...
    let pin_read = match pin.dir
    {
        0 => dp.GPIO0.data.read().bits(),
        1 => dp.GPIO1.data.read().bits(),
        2 => dp.GPIO2.data.read().bits(),
        3 => dp.GPIO3.data.read().bits(),
        _ => return Err(PinError::InvalidPort)    
    };
